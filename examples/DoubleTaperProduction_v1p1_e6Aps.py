...
def run_example():
    ...
    # Add additional rows of many apertures between device rows
    ...
    for i, x in enumerate(col_coords):
        ...
        for x_ap0 in np.linspace(-20, 20, 40):
            device_length/len(aperture_sizes)
            for x_ap1 in np.linspace()
            row_coords = np.linspace(
                wf_width / 2 - margin_y - offset * (i + 0.5),
                -wf_width / 2 + margin_y + offset * (num_cols - (i + 0.5) - 1),
                num_rows)
            for j, y in enumerate(row_coords):
                if(j == 3 or j == 7):
                    aperture_ref = write_field.add_ref(implant_window_single_dev)
                    aperture_ref.move([x + x_ap + 0.624 * photonic_scaling[i], y])
                else:
                    aperture_ref = write_field.add_ref(implant_window_dev)
                    aperture_ref.move([x  + x_ap + 0.736 * photonic_scaling[i], y])
    ...
    write_field.write_gds('Ovrcpld_v1p1_e6Apertures.gds')
